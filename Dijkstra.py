# Code by Eduard Tatievski
# For more information contact edi9200@gmail.com
...
class Graph:
    def __init__(self, filename):
        """Each line of the graph definition file defines an edge by specifying the start node, end node, and distance, delimited by spaces.

        Stores the graph definition in two properties which are used by Dijkstra's algorithm in the shortest_path method:
        self.nodes = set of all nodes in the graph
        self.adjacency_list = dict that maps each node to an unordered set of (neighbor, distance) tuples."""
    ...
    def shortest_path(self, start_node, end_node):
        """Uses Dijkstra's algorithm to determine the shortest path from start_node to end_node. Returns (path, distance)."""
...
def main():
    ...
    verify_algorithm(filename="testData.txt", start="Sat124", end="Sat2404")
    #verify_algorithm(filename="new_list.txt", start="satellite1", end="satellite45")
    #verify_algorithm(filename="new_list.txt", start="satellite241", end="satellite289")
